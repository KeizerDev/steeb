...
def get_albums(evt):
    result = musicbrainzngs.get_artist_by_id(evt.target.get_selected_items()[0]['id'], includes=["release-groups"], release_type=["album", "ep"])
    resultList = []
...
with gui.Window(name='mywin', title=u'Steeb', height=win_height, width=win_width, left='323', top='137', bgcolor=u'#F0F0F0', fgcolor=u'#555555', image='', ):
    ...
    with gui.ListView(name='artistslist', height=lv_artist_height, left='0', top=form_height, width=win_width, item_count=10, sort_column=0, onitemselected="print ('sel %s' % event.target.get_selected_items())", ):
        gui.ListColumn(name='artist', text='Artist', width=400)
        gui.ListColumn(name='id', text='Id', width=0)
    with gui.ListView(name='albumslist', height=lv_albums_height, left='0', top=lv_artist_top, width=win_width, item_count=10, sort_column=0, onitemselected="print ('sel %s' % event.target.get_selected_items())", ):
